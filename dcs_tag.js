...
var gDLTrTypes="xls,doc,pdf,swf,exe,wmv,ppt,xlsx,mp4,docx,pptx,pkg";
function dcsUrlIsDLTracked(durl){
    return dcsUrlExtIsDLTracked(durl.substring(durl.lastIndexOf(".")+1,durl.length));
}
function dcsUrlExtIsDLTracked(durlext){
    var adltrext=gDLTrTypes.split(',');
    for (var i=0;i<adltrext.length;i++){
		if (durlext == adltrext[i]){
		       return 1;
		}
    }
    return 0;
...
//			var types="xls,doc,pdf,swf,exe,wmv,ppt,xlsx,mp4,docx,pptx,pkg";
			if (dcsUrlExtIsDLTracked(e.pathname.substring(e.pathname.lastIndexOf(".")+1,e.pathname.length))){
